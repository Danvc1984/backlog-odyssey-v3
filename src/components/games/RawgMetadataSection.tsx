...
function Values({ label, values }: { label: string; values: readonly string[] }) {
  ...
  return <div><dt className="text-xs text-muted-foreground">{label}</dt><dd className="mt-1 flex flex-wrap gap-1.5">{values.map((value) => <span key={value} className="rounded-md border border-border bg-muted/30 px-2 py-0.5 text-xs">{value}</span>)}</dd></div>;
}

export function RawgMetadataSection({ payload, sourceUrl, fetchedAt }: { payload: RawgMetadataPayload | null; sourceUrl: string | null; fetchedAt: Date | null }) {
  const source = externalUrl(sourceUrl ?? payload?.rawgUrl ?? null);
  const website = externalUrl(payload?.website ?? null);
  return <SectionCard eyebrow="Metadata" title="Game details" id="rawg-metadata-heading">
    {!payload ? <div className="rounded-lg border border-dashed border-border p-4"><p className="text-sm font-medium">No metadata yet.</p><p className="mt-1 text-sm text-muted-foreground">Metadata will appear here after this game is matched.</p></div> : <div className="space-y-5 rounded-lg border border-border p-4">
      {payload.description && <p className="whitespace-pre-line text-sm leading-6 text-muted-foreground">{payload.description}</p>}
      ...
      <div className="flex flex-col gap-3 border-t border-border pt-3 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 sm:justify-end"><span>Last updated{fetchedAt ? ` ${fetchedAt.toLocaleDateString("en-US")}` : ""}.</span>{source ? <a href={source} target="_blank" rel="noreferrer" className="underline-offset-4 hover:underline"><span className="inline-flex items-center gap-1">RAWG <ArrowSquareOutIcon aria-hidden="true" className="size-3" /></span></a> : <span>Source link unavailable.</span>}</div>
      </div>
    </div>}
  </SectionCard>;
}
