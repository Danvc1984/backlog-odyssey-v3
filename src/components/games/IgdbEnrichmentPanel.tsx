...
interface IgdbEnrichmentPanelProps {
  gameId: string;
  catalogName: string;
  initialJob: IgdbEnrichmentJobView | null;
  hasIgdbSnapshot: boolean;
  igdbTitle: string | null;
}

interface JobEndpointResult {
  success: boolean;
  data: IgdbEnrichmentJobView | null;
  error: string | null;
}
...
function releaseDate(value: string | null): string {
  return value ? new Date(value).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" }) : "Release date unavailable";
}

export function IgdbEnrichmentPanel({ gameId, catalogName, initialJob, hasIgdbSnapshot, igdbTitle }: IgdbEnrichmentPanelProps) {
  ...
  const requestJobStatus = useCallback(async (jobId: string) => {
    const response = await fetch(`/api/enrichment/igdb/${encodeURIComponent(jobId)}`, { cache: "no-store" });
    const result = (await response.json()) as JobEndpointResult;
    if (!response.ok || !result.success || !result.data) throw new Error(result.error ?? "Failed to load IGDB enrichment status");
    return result.data;
  }, []);
  ...
  const runJob = useCallback(async (jobId: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/enrichment/igdb/${encodeURIComponent(jobId)}`, { method: "POST" });
      const result = (await response.json()) as JobEndpointResult;
      if (!response.ok || !result.success || !result.data) throw new Error(result.error ?? "Failed to run IGDB enrichment");
      setJob(result.data);
      if (result.data.status === "SUCCEEDED") router.refresh();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Failed to run IGDB enrichment");
    } finally {
      setRunning(false);
    }
  }, [router]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (cancelled) return;
        setJob(latest);
        const retryDue = latest.status === "RETRY_WAIT" && (!latest.nextAttemptAt || new Date(latest.nextAttemptAt).getTime() <= Date.now());
        if (latest.status === "QUEUED" || retryDue) await runJob(latest.id);
      } catch (caught) {
        if (!cancelled) setError(caught instanceof Error ? caught.message : "Failed to poll IGDB enrichment");
      }
    };
    void poll();
  ...
  const candidatePageCount = job?.status === "AWAITING_MATCH" ? Math.max(1, Math.ceil(job.candidates.length / CANDIDATES_PER_PAGE)) : 1;
  const visibleCandidatePageIndex = Math.min(candidatePageIndex, candidatePageCount - 1);
  const visibleCandidates = job?.status === "AWAITING_MATCH" ? job.candidates.slice(visibleCandidatePageIndex * CANDIDATES_PER_PAGE, (visibleCandidatePageIndex + 1) * CANDIDATES_PER_PAGE) : [];
  const hasPreviousCandidatePage = visibleCandidatePageIndex > 0;
  const hasNextCandidatePage = job?.status === "AWAITING_MATCH" && (visibleCandidatePageIndex + 1) * CANDIDATES_PER_PAGE < job.candidates.length;

  const startEnrichment = async (confirmOverwrite: boolean) => {
    setRunning(true); setError(null);
    try {
      const result = await requestIgdbEnrichment({ gameId, confirmOverwrite });
      if (!result.success) throw new Error(result.error ?? "Failed to queue IGDB enrichment");
      if (result.data.kind === "OVERWRITE_REQUIRED") { setPendingOverwrite(true); return; }
      setPendingOverwrite(false); setJob(result.data.job); await runJob(result.data.job.id);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Failed to queue IGDB enrichment"); }
  ...
  const startMatchReview = async () => {
    setRunning(true); setError(null);
    try {
      const result = await requestIgdbMatchReview({ gameId, title: catalogName });
      if (!result.success) throw new Error(result.error ?? "Failed to search IGDB matches");
      setPendingOverwrite(false); setCandidatePageIndex(0); setSelectedCandidateId(null); setJob(result.data.job);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Failed to search IGDB matches"); }
    finally { setRunning(false); }
  };

  const chooseMatch = async (igdbId: number) => {
    if (!job) return;
    setSelectedCandidateId(igdbId); setRunning(true); setError(null);
    try {
      const result = await selectIgdbMatch({ jobId: job.id, igdbId });
      if (!result.success) throw new Error(result.error ?? "Failed to select IGDB match");
      setJob(result.data.job); await runJob(result.data.job.id);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Failed to select IGDB match"); }
    finally { setRunning(false); }
  };
  ...
  const nextCandidatePage = async () => {
    ...
    if (nextStart < job.candidates.length) { setCandidatePageIndex(visibleCandidatePageIndex + 1); return; }
    if (!job.hasMoreCandidates) return;
    setRunning(true); setError(null);
    try {
      const result = await loadMoreIgdbCandidates({ jobId: job.id });
      if (!result.success) throw new Error(result.error ?? "Failed to load more IGDB matches");
      setJob(result.data.job);
      if (result.data.job.candidates.length > job.candidates.length) setCandidatePageIndex(visibleCandidatePageIndex + 1);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Failed to load more IGDB matches"); }
    finally { setRunning(false); }
  };
  ...
  const cancelMatchReview = async () => {
    ...
    setRunning(true); setError(null);
    try {
      const result = await cancelIgdbEnrichment({ jobId: job.id });
      if (!result.success) throw new Error(result.error ?? "Failed to cancel IGDB enrichment");
      setJob(result.data.job);
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Failed to cancel IGDB enrichment"); }
    finally { setRunning(false); }
  };
  ...
  return (
    <SectionCard eyebrow="Maintenance" title="Enrichment" id="igdb-enrichment-heading" description="Refresh matched game information without changing your catalog name automatically." status={<StatusPill tone={job?.status === "FAILED" ? "danger" : job ? "warning" : "neutral"}>{job?.status?.replaceAll("_", " ") ?? "Ready"}</StatusPill>}>
      <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex flex-wrap gap-2">
        ...
        {canStart && !pendingOverwrite && hasIgdbSnapshot && <Button type="button" size="icon" variant="ghost" disabled={running} onClick={() => void startEnrichment(false)} aria-label="Refresh IGDB metadata" title="Refresh IGDB metadata"><ArrowClockwiseIcon aria-hidden="true" className={running ? "animate-spin" : undefined} /></Button>}
      </div></div>
      ...
      {job && <div className="mt-4 space-y-3 text-sm"><div className="flex items-center justify-between gap-3"><p>{jobMessage(job)}</p><span className="rounded-md border border-border px-2 py-0.5 text-xs font-medium">{job.status.replaceAll("_", " ")}</span></div>
        ...
        {job.status === "FAILED" && <Button type="button" size="sm" variant="outline" disabled={running} onClick={() => void startEnrichment(false)}>Try again</Button>}
      </div>}
