...
describe("recommendationCopy", () => {
  it("composes a sentence from opener and caveat labels", () => {
    ...
    ).toBe("A steady course for your backlog: high interest.");
  });
  ...
  it("caps the reasons at two and favors caveats first", () => {
    expect(
      ...
      "A different current when you want a reset: runs well on Linux · short session.",
    );
  });
  ...
  it("fills with a positive factor when there are no caveats", () => {
    ...
    ).toBe("A favorable tide for your wallet: fresh 20% discount.");
  });
  ...
  it("falls back to a kind-based opener without a role", () => {
    ...
    ).toBe("A promising find for your wallet: under your target price.");
  });
